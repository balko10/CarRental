...
@Component({
  ...
  template: `
    <h2 mat-dialog-title style="margin-bottom: 1.5rem; text-align:center; font-weight:600;">Kunde bearbeiten</h2>
...
export class EditCustomerDialogComponent {
  ...
  onSave() {
    if (!this.customer.customerId || !this.customer.firstname || !this.customer.lastname || !this.customer.email) return;
    this.loading = true;
    this.customerService.updateCustomer(this.customer.customerId,this.customer as CustomerDto).subscribe({
      next: () => {
