...
@Component({
  ...
  template: `
    <h2 mat-dialog-title style="margin-bottom: 1.5rem; text-align:center; font-weight:600; letter-spacing:0.02em;">Neues Auto anlegen</h2>
...
export class CreateCarDialogComponent {
  ...
  constructor(
    private dialogRef: MatDialogRef<CreateCarDialogComponent>,
    private carService: CarControllerService
  ) {}

  onSave() {
    if (!this.car.brand || !this.car.model || !this.car.dailyPrice) return;
    this.loading = true;
    this.carService.createCar(this.car as CarDto).subscribe({
      next: () => {
